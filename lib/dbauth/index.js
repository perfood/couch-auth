...
module.exports = function (config, userDB, couchAuthDB) {
  ...
  this.authorizeUserSessions = function (
    user_id,
    personalDBs,
    sessionKeys,
    roles
    ...
    Object.keys(personalDBs).forEach(function (personalDB) {
      ...
      var db = new PouchDB(
        util.getDBURL(config.getItem('dbServer')) + '/' + personalDB
      );
      promises.push(
        self.authorizeKeys(user_id, db, sessionKeys, permissions, roles)
      );
    });
  ...
  this.addUserDB = function (
    ...
    var prefix = config.getItem('userDBs.privatePrefix')
    ...
    return userDB
      ...
      .then(function (results) {
        ...
        results.rows.forEach(function (row) {
          ...
          if (userDocs[row.value.user].session) {
            Object.keys(userDocs[row.value.user].session).forEach(function (
              session
            ) {
      ...
      .then(function () {
        ...
        Object.keys(keysByUser).forEach(function (user) {
          deauthorize.push(
            self.deauthorizeUser(userDocs[user], keysByUser[user])
          );
        });
  ...
  this.getDBConfig = function (dbName, type) {
    ...
    dbConfig.adminRoles =
      config.getItem('userDBs.defaultSecurityRoles.admins') || [];
    dbConfig.memberRoles =
      config.getItem('userDBs.defaultSecurityRoles.members') || [];
    ...
    if (config.getItem(dbConfigRef)) {
      ...
      dbConfig.type =
        type || config.getItem(dbConfigRef + '.type') || 'private';
      ...
      if (!type || type === 'private') {
        dbConfig.designDocs =
          config.getItem('userDBs.model._default.designDocs') || [];
      } else {
        dbConfig.designDocs = [];
      }
  ...
  this.removeDB = function (dbName) {
    var db = new PouchDB(
      util.getDBURL(config.getItem('dbServer')) + '/' + dbName
    );
    return db.destroy();
  };
