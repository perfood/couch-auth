...
var _dbAuth, _session, _onCreateActions, _onLinkActions;
...
class User {
    constructor(config, userDB, couchAuthDB, mailer, emitter) {
        _dbAuth.set(this, void 0);
        _session.set(this, void 0);
        _onCreateActions.set(this, void 0);
        _onLinkActions.set(this, void 0);
        this.userDB = userDB;
        this.config = config;
        this.mailer = mailer;
        this.emitter = emitter;
        const dbAuth = new dbauth_1.DBAuth(config, userDB, couchAuthDB);
        __classPrivateFieldSet(this, _dbAuth, dbAuth);
        __classPrivateFieldSet(this, _session, new session_1.Session(config));
        __classPrivateFieldSet(this, _onCreateActions, []);
        __classPrivateFieldSet(this, _onLinkActions, []);
        ...
        this.passwordConstraints = deepmerge_1.default(this.passwordConstraints, additionalConstraints ? additionalConstraints : {});
        ...
        this.validateUsername = function (username) {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                try {
                    const result = yield userDB.view('auth', 'username', { key: username });
        ...
        this.validateEmail = function (email) {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                try {
                    const result = yield userDB.view('auth', 'email', { key: email });
                    if (result.rows.length === 0) {
                        // Pass!
                        return;
                    }
        ...
        this.validateEmailUsername = function (email) {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                try {
                    const result = yield userDB.view('auth', 'emailUsername', {
                        key: email
                    });
                    if (result.rows.length === 0) {
                        return;
    ...
    onCreate(fn) {
        if (typeof fn === 'function') {
            __classPrivateFieldGet(this, _onCreateActions).push(fn);
        }
        else {
    ...
    onLink(fn) {
        if (typeof fn === 'function') {
            __classPrivateFieldGet(this, _onLinkActions).push(fn);
        }
        else {
    ...
    createUser(form, req) {
        ...
        if (typeof newUserModel === 'object') {
            ...
            if (newUserModel.whitelist) {
                whitelist = util_1.arrayUnion(this.userModel.whitelist, newUserModel.whitelist);
            }
            const addUserModel = this.config.getItem('userModel');
            finalUserModel = deepmerge_1.default(this.userModel, addUserModel ? addUserModel : {});
            finalUserModel.whitelist = whitelist || finalUserModel.whitelist;
        }
        const UserModel = sofa_model_1.default(finalUserModel);
        ...
        return user
            ...
            if (this.config.getItem('local.sendConfirmEmail')) {
                newUser.unverifiedEmail = {
                    email: newUser.email,
                    token: util_1.URLSafeUUID()
                };
                delete newUser.email;
            }
            return util_1.hashPassword(newUser.password);
        })
            .catch(err => {
            ...
            .then(newUser => {
            return this.processTransformations(__classPrivateFieldGet(this, _onCreateActions), newUser, 'local');
        })
            .then(finalNewUser => {
            ...
            return this.mailer.sendEmail('confirmEmail', newUser.unverifiedEmail.email, { req: req, user: newUser });
        })
            .then(() => {
    ...
    socialAuth(provider, auth, profile, req) {
        ...
        return Promise.resolve()
            ...
            .then(userDoc => {
            if (newAccount) {
                return this.processTransformations(__classPrivateFieldGet(this, _onCreateActions), userDoc, provider);
            }
            else {
                return this.processTransformations(__classPrivateFieldGet(this, _onLinkActions), userDoc, provider);
            }
        })
            .then(finalUser => {
    ...
    unlink(user_id, provider) {
        ...
        return this.userDB
            ...
            if (!user[provider] || typeof user[provider] !== 'object') {
                return Promise.reject({
                    error: 'Unlink failed',
                    message: 'Provider: ' + util_1.capitalizeFirstLetter(provider) + ' not found.',
                    status: 404
                });
            }
    ...
    createSession(user_id, provider, req = {}) {
        return __awaiter(this, void 0, void 0, function* () {
            const ip = req.ip;
            let user = yield this.userDB.get(user_id);
            const token = yield this.generateSession(user._id, user.roles);
            const password = token.password;
            const newToken = token;
            newToken.provider = provider;
            yield __classPrivateFieldGet(this, _session).storeToken(newToken);
            yield __classPrivateFieldGet(this, _dbAuth).storeKey(user_id, newToken.key, password, newToken.expires, user.roles, provider);
            // authorize the new session across all dbs
            if (user.personalDBs) {
                yield __classPrivateFieldGet(this, _dbAuth).authorizeUserSessions(user_id, user.personalDBs, newToken.key, user.roles);
            }
            if (!user.session) {
                user.session = {};
            }
            ...
            const userDoc = yield this.logActivity(user._id, 'login', provider, req, user);
            // Clean out expired sessions on login
            const finalUser = yield this.logoutUserSessions(userDoc, 'expired');
            user = finalUser;
            yield this.userDB.insert(finalUser);
            ...
            if (typeof user.personalDBs === 'object') {
                ...
                if (this.config.getItem('dbServer.publicURL')) {
                    const dbObj = url_1.default.parse(this.config.getItem('dbServer.publicURL'));
    ...
    refreshSession(key) {
        let newSession;
        return __classPrivateFieldGet(this, _session).fetchToken(key)
            .then(oldToken => {
            newSession = oldToken;
            newSession.expires = Date.now() + this.sessionLife * 1000;
            return Promise.all([
                this.userDB.get(newSession._id),
                __classPrivateFieldGet(this, _session).storeToken(newSession)
            ]);
        })
            .then(results => {
    ...
    resetPassword(form, req = undefined) {
        req = req || {};
        const ResetPasswordModel = sofa_model_1.default(this.resetPasswordModel);
        const passwordResetForm = new ResetPasswordModel(form);
        let user;
        return passwordResetForm
            .validate()
            .then(() => {
            const tokenHash = util_1.hashToken(form.token);
            ...
            return util_1.hashPassword(form.password);
        })
            .then(hash => {
            if (!user.local) {
                user.local = {};
            ...
            if (user.unverifiedEmail) {
                user = yield this.markEmailAsVerified(user, 'verified via password reset', req);
            }
    ...
    changePasswordSecure(user_id, form, req) {
        return __awaiter(this, void 0, void 0, function* () {
            req = req || {};
            const ChangePasswordModel = sofa_model_1.default(this.changePasswordModel);
            const changePasswordForm = new ChangePasswordModel(form);
            try {
                yield changePasswordForm.validate();
            }
            catch (err) {
                throw {
                    error: 'Validation failed',
            ...
            try {
                const user = yield this.userDB.get(user_id);
                if (user.local && user.local.salt && user.local.derived_key) {
                    ...
                    yield util_1.verifyPassword(user.local, form.currentPassword);
                }
                yield this.changePassword(user._id, form.newPassword, user, req);
            }
            catch (err) {
                throw (err || {
                    error: 'Password change failed',
    ...
    changePassword(user_id, newPassword, userDoc, req) {
        ...
        return promise
            .then(doc => {
            user = doc;
            return util_1.hashPassword(newPassword);
        }, err => {
            return Promise.reject({
            ...
            return this.logActivity(user._id, 'changed password', 'local', req, user);
        })
            .then(finalUser => this.userDB.insert(finalUser))
            .then(() => this.sendModifiedPasswordEmail(user, req))
            .then(() => {
    ...
    sendModifiedPasswordEmail(user, req) {
        if (this.config.getItem('local.sendPasswordChangedEmail')) {
            return this.mailer.sendEmail('modifiedPassword', user.email || user.unverifiedEmail.email, { user: user, req: req });
        }
        else {
            return Promise.resolve();
        }
    ...
    forgotPassword(email, req) {
        ...
        return this.userDB
            ...
            if (this.config.getItem('local.tokenLengthOnReset')) {
                token = token.substring(0, this.config.getItem('local.tokenLengthOnReset'));
            }
            tokenHash = util_1.hashToken(token);
            user.forgotPassword = {
                token: tokenHash,
                issued: Date.now(),
                expires: Date.now() + this.tokenLife * 1000
            };
            return this.logActivity(user._id, 'forgot password', 'local', req, user);
        })
            .then(finalUser => {
            return this.userDB.insert(finalUser);
        })
            .then(() => {
            return this.mailer.sendEmail('forgotPassword', user.email || user.unverifiedEmail.email, { user: user, req: req, token: token }); // Send user the unhashed token
        })
            .then(() => {
    ...
    changeEmail(user_id, newEmail, req) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const emailError = yield this.validateEmail();
            if (emailError) {
                throw emailError;
            }
            const user = yield this.userDB.get(user_id);
            if (this.config.getItem('local.sendConfirmEmail')) {
                ...
                const mailType = this.config.getItem('emails.confirmEmailChange')
                    ? 'confirmEmailChange'
                    : 'confirmEmail';
                yield this.mailer.sendEmail(mailType, user.unverifiedEmail.email, {
                    req: req,
                    user: user
                });
            }
            ...
            const finalUser = yield this.logActivity(user._id, 'changed email', req.user.provider, req, user);
            return this.userDB.insert(finalUser);
        });
    }
    addUserDB(user_id, dbName, type, designDocs, permissions) {
        let userDoc;
        const dbConfig = __classPrivateFieldGet(this, _dbAuth).getDBConfig(dbName, type || 'private');
        ...
        return this.userDB
            ...
            .then(result => {
            userDoc = result;
            return __classPrivateFieldGet(this, _dbAuth).addUserDB(userDoc, dbName, dbConfig.designDocs, dbConfig.type, dbConfig.permissions, dbConfig.adminRoles, dbConfig.memberRoles);
        })
            .then(finalDBName => {
                ...
                for (const db of Object.keys(user.personalDBs)) {
                    if (user.personalDBs[db].name === dbName) {
                        ...
                        if (type === 'private' && deletePrivate) {
                            yield __classPrivateFieldGet(this, _dbAuth).removeDB(dbName);
                        }
    ...
    logoutSession(session_id) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let startSessions = 0;
            let endSessions = 0;
            const results = yield this.userDB.view('auth', 'session', {
                key: session_id,
                include_docs: true
            });
            if (!results.rows.length) {
            ...
            const promises = [];
            promises.push(__classPrivateFieldGet(this, _session).deleteTokens(session_id));
            promises.push(__classPrivateFieldGet(this, _dbAuth).removeKeys(session_id));
            if (user) {
                promises.push(__classPrivateFieldGet(this, _dbAuth).deauthorizeUser(user, session_id));
            }
            yield Promise.all(promises);
            // Clean out expired sessions
            const finalUser = yield this.logoutUserSessions(user, 'expired');
            user = finalUser;
            if (user.session) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (results.rows.length) {
                ...
                if (user.session && user.session[session_id]) {
                    const finalUser = yield this.logoutUserSessions(user, 'other', session_id);
                    return this.userDB.insert(finalUser);
                }
            }
            return false;
    ...
    logoutUserSessions(userDoc, op, currentSession) {
        ...
        let sessions;
        if (op === 'all' || op === 'other') {
            sessions = util_1.getSessions(userDoc);
        }
        else if (op === 'expired') {
            sessions = util_1.getExpiredSessions(userDoc, Date.now());
        }
        ...
        if (sessions.length) {
            ...
            promises.push(__classPrivateFieldGet(this, _session).deleteTokens(sessions));
            // Remove the keys from our couchDB auth database
            promises.push(__classPrivateFieldGet(this, _dbAuth).removeKeys(sessions));
            // Deauthorize keys from each personal database
            promises.push(__classPrivateFieldGet(this, _dbAuth).deauthorizeUser(userDoc, sessions));
    ...
    removeUser(user_id, destroyDBs) {
        return __awaiter(this, void 0, void 0, function* () {
            const promises = [];
            const userDoc = yield this.userDB.get(user_id);
            const user = yield this.logoutUserSessions(userDoc, 'all');
            if (destroyDBs !== true || !user.personalDBs) {
                return Promise.resolve();
            }
            Object.keys(user.personalDBs).forEach(userdb => {
                if (user.personalDBs[userdb].type === 'private') {
                    promises.push(__classPrivateFieldGet(this, _dbAuth).removeDB(userdb));
                }
            });
            yield Promise.all(promises);
    ...
    confirmSession(key, password) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                return yield __classPrivateFieldGet(this, _session).confirmToken(key, password);
            }
            catch (error) {
                if (this.useDbFallback) {
                    try {
                        const doc = yield __classPrivateFieldGet(this, _dbAuth).retrieveKey(key);
                        if (doc.expires > Date.now()) {
                            ...
                            yield __classPrivateFieldGet(this, _session).storeToken(token);
                            return __classPrivateFieldGet(this, _session).confirmToken(key, password);
                        }
                    }
                    catch (_a) { }
                }
            }
            throw session_1.Session.invalidMsg;
        });
    }
    removeFromSessionCache(keys) {
        return __classPrivateFieldGet(this, _session).deleteTokens(keys);
    }
    quitRedis() {
        return __classPrivateFieldGet(this, _session).quit();
    }
    ...
    addUserDBs(newUser) {
        ...
        const processUserDBs = (dbList, type) => {
            dbList.forEach(userDBName => {
                const dbConfig = __classPrivateFieldGet(this, _dbAuth).getDBConfig(userDBName);
                promises.push(__classPrivateFieldGet(this, _dbAuth).addUserDB(newUser, userDBName, dbConfig.designDocs, type, dbConfig.permissions, dbConfig.adminRoles, dbConfig.memberRoles)
                    .then(finalDBName => {
                    delete dbConfig.permissions;
                    delete dbConfig.adminRoles;
                    delete dbConfig.memberRoles;
    ...
    removeExpiredKeys() {
        return __classPrivateFieldGet(this, _dbAuth).removeExpiredKeys();
    }
}
exports.User = User;
_dbAuth = new WeakMap(), _session = new WeakMap(), _onCreateActions = new WeakMap(), _onLinkActions = new WeakMap();
