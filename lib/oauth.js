...
module.exports = function (router, passport, user, config) {
    ...
    function linkTokenSuccess(req, res, next) {
        ...
        res.status(200).json({
            ok: true,
            success: util_1.capitalizeFirstLetter(provider) + ' successfully linked',
            provider: provider
        });
    }
    ...
    function registerProvider(provider, configFunction) {
        ...
        if (config.getItem(configRef + '.credentials')) {
            ...
            if (!config.getItem('security.disableLinkAccounts')) {
                ...
                router.get('/link/' + provider + '/callback', passport.authenticate('bearer', { session: false }), passportCallback(provider, options, 'link'), linkSuccess, oauthErrorHandler);
            }
    ...
    function registerOAuth2(providerName, Strategy) {
        registerProvider(providerName, function (credentials, passport, authHandler) {
            passport.use(new Strategy(credentials, function (req, accessToken, refreshToken, profile, done) {
                util_2.callbackify(authHandler)(req, providerName, { accessToken: accessToken, refreshToken: refreshToken }, profile, done);
            }));
        });
    }
    ...
    function registerTokenProvider(providerName, Strategy) {
        ...
        if (config.getItem(configRef + '.credentials')) {
            ...
            passport.use(providerName + '-token', new Strategy(credentials, function (req, accessToken, refreshToken, profile, done) {
                util_2.callbackify(authHandler)(req, providerName, { accessToken: accessToken, refreshToken: refreshToken }, profile, done);
            }));
            router.post('/' + providerName + '/token', passportTokenCallback(providerName, options), initTokenSession, tokenAuthErrorHandler);
            if (!config.getItem('security.disableLinkAccounts')) {
                router.post('/link/' + providerName + '/token', passport.authenticate('bearer', { session: false }), passportTokenCallback(providerName, options), linkTokenSuccess, tokenAuthErrorHandler);
            }
