...
class SuperLogin extends user_1.User {
    constructor(configData, passport, userDB, couchAuthDB) {
        ...
        if (!userDB && config.getItem('dbServer.userDB')) {
            userDB = nano_1.default(util_1.getDBURL(config.getItem('dbServer'))).use(config.getItem('dbServer.userDB'));
        }
        if (!couchAuthDB &&
            config.getItem('dbServer.couchAuthDB') &&
            !config.getItem('dbServer.cloudant')) {
            couchAuthDB = nano_1.default(util_1.getDBURL(config.getItem('dbServer'))).use(config.getItem('dbServer.couchAuthDB'));
        }
