...
module.exports = function (config, router, passport, user) {
    ...
    const requirePass = config.getItem('local.requirePasswordOnEmailChange') === true;
    ...
    router.post('/login', function (req, res, next) {
        ...
        return user.createSession(req.user._id, 'local', req).then(function (mySession) {
    ...
    router.post('/logout', function (req, res, next) {
        const sessionToken = util_1.getSessionToken(req);
        if (!sessionToken) {
            return next({
                error: 'unauthorized',
                status: 401
    ...
    router.post('/logout-all', function (req, res, next) {
        ...
        user.logoutUser(null, sessionToken).then(function () {
    ...
    router.post('/unlink/:provider', passport.authenticate('bearer', { session: false }), function (req, res, next) {
        ...
        user.unlink(req.user._id, provider).then(function () {
            res.status(200).json({
                success: util_1.capitalizeFirstLetter(provider) + ' unlinked'
            });
        }, function (err) {
            return next(err);
        });
