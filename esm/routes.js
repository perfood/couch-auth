'use strict';
import { capitalizeFirstLetter, getSessionToken, isUserFacingError } from './util';
import { ValidErr } from './user';
export default function (config, router, passport, user) {
    ...
    if (!disabled.includes('login'))
        router.post('/login', function (req, res, next) {
            ...
            return user.createSession(req.user._id, 'local', true).then(function (mySession) {
        ...
        router.post('/register', function (req, res, next) {
            user.createUser(req.body, req).then(function (newUser) {
                if (!newUser || !config.security.loginOnRegistration) {
                    ...
                    return user.createSession(newUser._id, 'local', true).then(function (mySession) {
                        res.status(200).json(mySession);
                    }, function (err) {
                        return next(err);
                    });
        ...
        router.post('/password-reset', function (req, res, next) {
            user.resetPassword(req.body, req).then(function (currentUser) {
                if (config.security.loginOnPasswordReset) {
                    return user.createSession(currentUser._id, 'local', true).then(function (mySession) {
                        res.status(200).json(mySession);
                    }, function (err) {
                        return next(err);
                    });
